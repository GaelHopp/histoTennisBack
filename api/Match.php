...
function updateMatchLive($match){
	$db = getConnection();
	$idMatchLive = intval($match->idMatchLive);
	$player1 = $match->player1;
	$player2 = $match->player2;
	$set1Player1 = $match->set1Player1;
	$set1Player2 = $match->set1Player2;
	...
	$stmt = $db->query("UPDATE `MatchLive` SET player1 = '$player1', player2 = '$player2', set1Player1 = $set1Player1, 
												...
												set3Player1 = $set3Player1, set3Player2 = $set3Player2, player1Serving = $player1Serving 
...
function deleteMatchLive($id){
	$db = getConnection();
	$stmt = $db->query("DELETE FROM `MatchLive` WHERE idMatchLive = $id");
}
